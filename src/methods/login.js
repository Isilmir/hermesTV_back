  ...
  where p.id='${req.body.user}'`);*/
	...
	//console.log(result.recordset[0]);
		...
		if(password==req.body.password){
			let token = jwt.sign({user:req.body.user,exp: Math.floor(Date.now() / 1000) + (60),isAdmin:req.body.user==100083/*result.recordset[0]['sideId']==16333*/?true:false, permissions:JSON.parse(result.recordset[0]['permissions']).map(el=>el.value)},privateKey,{ algorithm: 'RS256'}/*,'password'*/)
